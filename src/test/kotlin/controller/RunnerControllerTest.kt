...
class RunnerControllerTest {
    ...
    fun `lint snippet with invalid language version`() {
        ...
        whenever(runnerService.lintSnippet("test", "code", "invalid_version", "testUser")).thenReturn(expectedResponse)

        val response = runnerController.lintSnippet(snippetRequest, principal)

        assertEquals(ResponseEntity.ok(expectedResponse), response)
        verify(runnerService).lintSnippet("test", "code", "invalid_version", "testUser")
    }
    ...
    fun `format snippet with valid content and version`() {
        ...
        whenever(runnerService.formatSnippet("code", "3.9", "testUser")).thenReturn(expectedResponse)

        val response = runnerController.formatSnippet(formatRequest, principal)

        assertEquals(ResponseEntity.ok(expectedResponse), response)
        verify(runnerService).formatSnippet("code", "3.9", "testUser")
    }
}
